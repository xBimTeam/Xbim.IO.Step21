...
namespace Xbim.IO.Step21
{
    public static class TextWriterExtensions
    {
        ...
        public static void WriteKeyword(this TextWriter writer, SyntaxKind kind)
        {
            ...
            if (text == null)
                throw new ArgumentException($"Invadlid argument {nameof(kind)}: {kind}");
        ...
        public static void WriteDiagnostics(this TextWriter writer, IEnumerable<Diagnostic> diagnostics)
        {
                                                  ...
                                                  .ThenBy(d => d.Location.Span.Length))
            {
                ...
                // Context is presented only if SourceText
                if (text is SourceText st)
                {                   
                    var span = diagnostic.Location.Span;
                    var lineIndex = st.GetLineIndex(span.Start);
                    var line = st.Lines[lineIndex];
                    var prefixSpan = TextSpan.FromBounds(line.Start, span.Start);
                    var suffixSpan = TextSpan.FromBounds(span.End, line.End);
                    var prefix = st.ToString(prefixSpan);
                    var error = st.ToString(span);
                    var suffix = st.ToString(suffixSpan);
                    writer.Write("    ");
                    writer.Write(prefix);
                    writer.SetForeground(ConsoleColor.DarkRed);
                    writer.Write(error);
